    ...
    <div class="panel">
        <div class="mb-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <p class="text-sm text-gray-500">
                {{ loading ? 'Cargando...' : `Mostrando ${users.length} de ${pagination.total} usuarios` }}
            </p>

            <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
                <input v-model="localSearch" type="text" class="form-input min-w-[220px]" placeholder="Buscar por nombre o correo"
        ...
        <div class="datatable">
            <vue3-datatable :rows="users" :columns="columns" :loading="loading" :totalRows="pagination.total"
...
const emit = defineEmits<{
    ...
    applyFilters: [filters: { projectId: string; search: string }];
}>();
...
const columns = ref([
    ...
    { field: 'email', title: 'Correo', sort: true },
    { field: 'dni', title: 'DNI', sort: false },
    ...
    { field: 'action', title: 'Acciones', sort: false, width: '110px' },
]);
...
const resolveProjectName = (value: UserItem) => {
